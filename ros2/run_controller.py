...
class Quadruped_PyMPC_Node(Node):
    def __init__(self):
        ...
        self.subscription_blind_state = self.create_subscription(
            BlindStateMsg, "/dls2/blind_state", self.get_blind_state_callback, qos
        )
        self.subscription_joy = self.create_subscription(
            Joy, "joy", self.get_joy_callback, 1
        )
        self.publisher_effors = self.create_publisher(
            Float64MultiArray, "effort_controller/commands", 1
        )
        self.publisher_control_signal = self.create_publisher(
        ...
        # Desired PD gain
        self.impedence_joint_position_gain = (
            np.ones(12) * cfg.simulation_params["impedence_joint_position_gain"]
        )
        self.impedence_joint_velocity_gain = (
            np.ones(12) * cfg.simulation_params["impedence_joint_velocity_gain"]
        )
        ...
        self.feet_traj_geom_ids, self.feet_GRF_geom_ids = None, LegsAttr(
            FL=-1, FR=-1, RL=-1, RR=-1
        )
        ...
        from quadruped_pympc.interfaces.srbd_batched_controller_interface import (
            SRBDBatchedControllerInterface,
        )
        from quadruped_pympc.interfaces.wb_interface import WBInterface

        self.wb_interface = WBInterface(
            initial_feet_pos=self.env.feet_pos(frame="world"),
            legs_order=self.legs_order,
        )
        ...
        self.nmpc_GRFs = LegsAttr(
            FL=np.zeros(3), FR=np.zeros(3), RL=np.zeros(3), RR=np.zeros(3)
        )
        self.nmpc_footholds = LegsAttr(
            FL=np.zeros(3), FR=np.zeros(3), RL=np.zeros(3), RR=np.zeros(3)
        )
        self.nmpc_joints_pos = LegsAttr(
            FL=np.zeros(3), FR=np.zeros(3), RL=np.zeros(3), RR=np.zeros(3)
        )
        self.nmpc_joints_vel = LegsAttr(
            FL=np.zeros(3), FR=np.zeros(3), RL=np.zeros(3), RR=np.zeros(3)
        )
        self.nmpc_joints_acc = LegsAttr(
            FL=np.zeros(3), FR=np.zeros(3), RL=np.zeros(3), RR=np.zeros(3)
        )
        ...
        self.tau_limits = LegsAttr(
            FL=self.env.mjModel.actuator_ctrlrange[self.env.legs_tau_idx.FL]
            * tau_soft_limits_scalar,
            FR=self.env.mjModel.actuator_ctrlrange[self.env.legs_tau_idx.FR]
            * tau_soft_limits_scalar,
            RL=self.env.mjModel.actuator_ctrlrange[self.env.legs_tau_idx.RL]
            * tau_soft_limits_scalar,
            RR=self.env.mjModel.actuator_ctrlrange[self.env.legs_tau_idx.RR]
            * tau_soft_limits_scalar,
        )
    ...
    def get_base_state_callback(self, msg):
        ...
        if USE_SMOOTH_VELOCITY:
            self.linear_velocity = 0.5 * self.linear_velocity + 0.5 * np.array(
                msg.linear_velocity
            )
        else:
    ...
    def get_joy_callback(self, msg):
        ...
        self.env._ref_base_ang_yaw_dot = msg.axes[3] / 2.0  # Yaw
        ...
        if msg.buttons[8] == 1:
            ...
            os.system(
                'kill -9 $(ps -u | grep -m 1 hal | grep -o "^[^ ]* *[0-9]*" | grep -o "[0-9]*")'
            )
    ...
    def compute_control_callback(self):
        ...
        if (
            self.first_message_base_arrived == False
            and self.first_message_joints_arrived == False
        ):
            return
        ...
        if cfg.simulation_params["use_inertia_recomputation"]:
            inertia = (
                self.env.get_base_inertia().flatten()
            )  # Reflected inertia of base at qpos, in world frame
        else:
        ...
        joints_pos = LegsAttr(
            FL=qpos[7:10], FR=qpos[10:13], RL=qpos[13:16], RR=qpos[16:19]
        )
        ...
        control_signal_msg.torques = np.concatenate(
            [self.tau.FL, self.tau.FR, self.tau.RL, self.tau.RR], axis=0
        ).flatten()
        control_signal_msg.timestamp = (
            self.loop_time
        )  # self.loop_time#self.last_mpc_loop_time
        ...
        trajectory_generator_msg.joints_velocity = np.concatenate(
            [
                pd_target_joints_vel.FL,
                pd_target_joints_vel.FR,
                pd_target_joints_vel.RL,
                pd_target_joints_vel.RR,
            ],
            axis=0,
        ).flatten()
